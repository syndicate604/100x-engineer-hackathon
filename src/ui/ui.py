...
class MarketInsightUI:
    def __init__(self):
        st.set_page_config(
            page_title="Market Insight Generator",
            page_icon="🚀",
            layout="wide"
        )
    ...
    def _save_report_to_json(self, report, report_type):
        ...
        filename = os.path.join(
            self.temp_dir, 
            f"{self.session_id}_{report_type}_report.json"
        )
    ...
    def customer_onboarding(self):
        ...
        return domain, problem_statement, solution_approach if submitted else (None, None, None)
